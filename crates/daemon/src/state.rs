            ...
            .spawn(move || {
                ...
                while let Ok(event) = rx.recv() {
                    ...
                    if let BackendEvent::NodeAppeared(ref node) = event {
                        if let Some(action) = evaluate_rule(&this, &node.name) {
...
/// Snapshot the rule outcome for `node_name`. Returns an owned `Action`
/// so the graph lock can be taken without holding the rules lock.
fn evaluate_rule(state: &DaemonState, node_name: &str) -> Option<(String, soundworm_policy::rules::Action)> {
    let rules = state.rules.lock().ok()?;
    let r = rules.evaluate_rule(node_name)?;
